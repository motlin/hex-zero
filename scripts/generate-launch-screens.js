...
function drawSplashScreen(canvas, width, height) {
	...
	// Background gradient
	const bgGradient = ctx.createRadialGradient(centerX, centerY, 0, centerX, centerY, minDimension * 0.7);
...
async function generateIOSLaunchScreens() {
	...
	await fs.writeFile(path.join(splashDir, 'Contents.json'), JSON.stringify(contents, null, '\t'));
...
async function generateAndroidSplashScreens() {
	...
	await fs.writeFile(path.join(androidResDir, 'drawable', 'splash.png'), universalCanvas.toBuffer('image/png'));
...
<layer-list xmlns:android="http://schemas.android.com/apk/res/android">
	...
	await fs.writeFile(path.join(androidResDir, 'drawable', 'launch_background.xml'), launchBackgroundXml);
