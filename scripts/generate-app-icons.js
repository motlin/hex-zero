...
async function generateAndroidIcons() {
	...
	for (const config of ANDROID_SIZES) {
		...
		await fs.writeFile(path.join(iconDir, 'ic_launcher.png'), canvas.toBuffer('image/png'));
		console.log(`  ✅ Created ${config.name}/ic_launcher.png`);

		// Round icon (same design for now)
		await fs.writeFile(path.join(iconDir, 'ic_launcher_round.png'), canvas.toBuffer('image/png'));
...
<adaptive-icon xmlns:android="http://schemas.android.com/apk/res/android">
	...
	await fs.writeFile(path.join(androidResDir, 'mipmap-anydpi-v26', 'ic_launcher.xml'), adaptiveIconXml);
	await fs.writeFile(path.join(androidResDir, 'mipmap-anydpi-v26', 'ic_launcher_round.xml'), adaptiveIconXml);
	...
	for (const config of NOTIFICATION_SIZES) {
		...
		await fs.writeFile(path.join(iconDir, 'ic_notification.png'), canvas.toBuffer('image/png'));
