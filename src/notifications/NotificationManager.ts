...
function isNotificationConfig(value: unknown): value is NotificationConfig {
	return (
		typeof value === 'object' &&
		value !== null &&
		'dailyReminderEnabled' in value &&
		typeof value.dailyReminderEnabled === 'boolean' &&
		'dailyReminderTime' in value &&
		typeof value.dailyReminderTime === 'string' &&
		'streakReminderEnabled' in value &&
		typeof value.streakReminderEnabled === 'boolean' &&
		'inactivityDays' in value &&
		typeof value.inactivityDays === 'number'
	);
}

function isStoredNotificationData(value: unknown): value is StoredNotificationData {
	if (typeof value !== 'object' || value === null || !('config' in value) || !isNotificationConfig(value.config)) {
		return false;
	}
	return !('lastPlayedDate' in value) || typeof value.lastPlayedDate === 'string';
}
...
export class NotificationManager {
	...
	private setupListeners(): void {
		void LocalNotifications.addListener('localNotificationReceived', (_notification) => {});
	...
	updateLastPlayed(): void {
		...
		this.saveConfig();
		void this.scheduleNotifications();
	}
	...
	private loadConfig(): void {
		...
		if (saved !== null && saved !== '') {
			try {
				...
				if (!isStoredNotificationData(parsed)) {
					throw new Error('Invalid notification configuration');
				}
				this.config = parsed.config;
				if (parsed.lastPlayedDate !== undefined && parsed.lastPlayedDate !== '') {
